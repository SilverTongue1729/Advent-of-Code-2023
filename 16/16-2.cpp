...
void solve(){
  ...
  map<pair<pii,char>,pii> mpdir;  // dir, char -> new_dir
  mpdir[{{0,1},'\\'}] = {1,0};
  mpdir[{{0,1},'/'}] = {-1,0};
  mpdir[{{1,0},'\\'}] = {0,1};
  mpdir[{{1,0},'/'}] = {0,-1};
  ...
  auto is_vert = [](pii dir) -> bool { return dir.ss == 0; };
  auto is_hor = [](pii dir) -> bool { return dir.ff == 0; };
  
  auto cnt = [&](pii st_pos, pii st_dir) -> int {
    set<pii> vis;
    set<pair<pii,pii>> dp;
    
    auto rec = [&](auto self, pii pos, pii dir) -> void {
      if (pos.ff < 0 or pos.ff >= n or pos.ss < 0 or pos.ss >= m) return;
      if (dp.find({pos,dir}) != dp.end()) return;
      
      dp.insert({pos,dir});
      vis.insert(pos);
      
      if (grid[pos.ff][pos.ss] == '.') self(self, pos+dir, dir);
      if (grid[pos.ff][pos.ss] == '\\') self(self, pos+mpdir[{dir,'\\'}], mpdir[{dir,'\\'}]);
      if (grid[pos.ff][pos.ss] == '/') self(self, pos+mpdir[{dir,'/'}], mpdir[{dir,'/'}]);
      if (grid[pos.ff][pos.ss] == '|') {
        if (is_vert(dir)){
          self(self, pos+dir, dir);
          return;
        }
        self(self, pos+mpdir[{dir,'\\'}], mpdir[{dir,'\\'}]);
        self(self, pos+mpdir[{dir,'/'}], mpdir[{dir,'/'}]);
      }
      if (grid[pos.ff][pos.ss] == '-') {
        if (is_hor(dir)){
  ...
  // top row
  for_(i,0,m) ans = max(ans, cnt({0,i}, {1,0}));
  // bottom row
  for_(i,0,m) ans = max(ans, cnt({n-1,i}, {-1,0}));
  // left col
  for_(i,0,n) ans = max(ans, cnt({i,0}, {0,1}));
  // right col
  for_(i,0,n) ans = max(ans, cnt({i,m-1}, {0,-1}));
