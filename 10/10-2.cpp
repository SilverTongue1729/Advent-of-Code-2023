...
void solve(){
  ...
  */
  ...
  pii cur;
  for_(i,0,n) for_(j,0,m) if(grid[i][j] == 'S') cur = {i,j};
  
  vpii dirs = {{1,0},{-1,0},{0,1},{0,-1}};
  
  for (auto dir : dirs) {
    if (mp.find({dir,grid[cur.ff+dir.ff][cur.ss+dir.ss]}) == mp.end()) continue;
    
    auto dfs = [&](auto self, pii cur, pii dir) -> pii {
      vis[cur.ff][cur.ss] = 1;
      pii next = cur + dir;
      if (vis[next.ff][next.ss]) return dir;
      pii next_dir = mp[{dir,grid[next.ff][next.ss]}];
      return self(self,next,next_dir);
    }; 
    
    pii fin_dir = dfs(dfs,cur,dir);

    for (auto [k,v] : mp)
      if (k.ff == fin_dir and v == dir)
        grid[cur.ff][cur.ss] = k.ss;
        
    int ans=0;
    
    map<char,float> mpval = {{'|',1},{'-',0},{'L',0.5},{'J',-0.5},{'7',0.5},{'F',-0.5}};
        
    for_(i,0,n) {
      float cnt=0;
      for_(j,0,m) {
        if (vis[i][j]) cnt += mpval[grid[i][j]];
        else if (int(cnt)&1) ans++;
      }
    }
