...
void solve(){
  ...
  while (!cin.eof()) {
    ...
    vi v2;
    for_(i,0,5) for_(j,0,sz(v)) v2.pb(v[j]);
    v = v2;
                
    vvl dp(sz(s)+1,vl(sz(v)+1,-1));
    
    auto recur = [&](auto self, int i=0, int j=0) -> ll {
      // i is index of s, j is index of v
      ...
      if (s[i]!='#') cnt += self(self,i+1,j);
      
      if (j<sz(v) and i+v[j]<=sz(s)) {
        bool flag=true;
        for_(k,i,i+v[j]){
          if (s[k]=='.'){
            flag=false;
        ...
        if (i+v[j]<sz(s) and s[i+v[j]]=='#') flag=false;
        if (flag) cnt += self(self, min(i+v[j]+1, sz(s)), j+1);
      }
