...
void solve(){
  string s; cin>>s;  // L or R
  map<string,pair<string,string>> mp;
  
  while(!cin.eof()) {
    string key, temp, val1, val2; cin>>key>>temp>>val1>>val2;
    mp[key] = {val1.substr(1,sz(val1)-2),val2.substr(0,sz(val2)-1)};
  }

  vs cur;  // every key string in mp which ends with an A
  for (auto& [k,v]:mp) if (k.back() == 'A') cur.pb(k);
    

  ll ans=1;
  for (auto& c : cur) {
    int i;
    for (i=0; c.back() != 'Z'; i++) c = (s[i % sz(s)] == 'L') ? mp[c].ff : mp[c].ss;
    ans = ans*i/gcd(ans,1ll*i);
  }
