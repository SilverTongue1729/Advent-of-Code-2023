...
void solve(){
  ...
  int n = sz(v), m = sz(v[0]);
  auto issymbol = [&](int i, int j) -> bool{
    return (i!=0 and !isdigit(v[i-1][j]) and v[i-1][j]!='.') or
           (i!=n-1 and !isdigit(v[i+1][j]) and v[i+1][j]!='.') or
           (!isdigit(v[i][j]) and v[i][j]!='.');
  };
  ...
  for_(i,0,n) for_(j,0,m){
    if (isdigit(v[i][j])){
      if (cur==0 and j!=0 and issymbol(i,j-1)) part = true, mp[num].ff=1;
      if (issymbol(i,j) or (j!=m-1 and issymbol(i,j+1))) part = true, mp[num].ff=1;
      cur = cur*10 + (v[i][j]-'0');
      grid[i][j] = num;
      if (j!=m-1) continue;
    }
  ...
  auto gear_ratio = [&](int r, int c) -> int {
    set<int> s;
    for_(i,r-1,r+2) for_(j,c-1,c+2) if (i>=0 and i<n and j>=0 and j<m and grid[i][j] and mp[grid[i][j]].ff) s.insert(grid[i][j]);
    if (sz(s)!=2) return 0;
  ...
  for_(i,0,n) for_(j,0,m) if (v[i][j]=='*') ans += gear_ratio(i,j);
