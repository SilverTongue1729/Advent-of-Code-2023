...
void solve(){
  ...
  map<pii,int> dir_to_int = {{{0,1},0},{{1,0},1},{{0,-1},2},{{-1,0},3}};
  ...
  while (!q.empty()){
    ...
    if (r==n-1 and c==m-1) { cout<<dist<<nl; return; }
    ...
    for (auto ndir : dirs) {
      ...
      if (ndir == dir and cont<3) 
        q.push({dist+grid[nr][nc],pos+ndir,ndir,cont+1});
      else if (ndir != dir and ndir!=pii(-dir.ff,-dir.ss))
