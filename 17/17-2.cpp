...
void solve(){
  ...
  while (!cin.eof()){
    string str; cin>>str;
    grid.pb(vi());
    for (char c:str) grid.back().pb(c-'0');
  }
  
  int n = sz(grid), m = sz(grid[0]);
    
  using node = tuple<int,pii,pii,int>;
  priority_queue<node, vector<node>, greater<node>> q;
  
  q.push({grid[0][1],{0,1},{0,1},1});
  q.push({grid[1][0],{1,0},{1,0},1});
  
  vpii dirs = {{0,1},{1,0},{0,-1},{-1,0}};
  map<pii,int> dir_to_int = {{dirs[0],0},{dirs[1],1},{dirs[2],2},{dirs[3],3}};
    
  while (!q.empty()){
    auto top = q.top(); q.pop();
    auto [dist, pos, dir, cont] = top;
    auto [r, c] = pos;

    if (r==n-1 and c==m-1){
      if (cont>=4){ cout<<dist<<nl; return; }
      else continue;
    }

    if (vis[r][c][dir_to_int[dir]][cont]) continue;
    vis[r][c][dir_to_int[dir]][cont] = true;
    
    for (auto ndir : dirs) {
      auto [nr,nc] = pos+ndir;
      if (nr<0 or nr>=n or nc<0 or nc>=m) continue;
      if (ndir == dir and cont<10) 
        q.push({dist+grid[nr][nc],pos+ndir,ndir,cont+1});
      if (ndir != dir and cont>=4 and ndir!=pii(-dir.ff,-dir.ss))
        q.push({dist+grid[nr][nc],pos+ndir,ndir,1});
    }
  }
