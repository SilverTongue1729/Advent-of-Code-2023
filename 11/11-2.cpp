...
void solve(){
  ...
  vi prerow(n), precol(m);
  
  for_(i,1,n){
    if (count(all(grid[i]),'.') == m) prerow[i] = prerow[i-1] + 1;
    else prerow[i] = prerow[i-1];
  }
  
  for_(i,1,m){
    int cnt = 0;
    for_(j,0,n) if (grid[j][i] == '.') cnt++;
    if (cnt == n) precol[i] = precol[i-1]+1;
    else precol[i] = precol[i-1];
  }
  
  ll ans=0;
  
  auto dist = [&](ll x1, ll y1, ll x2, ll y2){
    return abs(x1-x2)+abs(y1-y2) + 999'999 * (abs(prerow[x2]-prerow[x1]) + abs(precol[y2]-precol[y1]));
  };
  
  vpii pos;
  for_(i,0,n) for_(j,0,m) if (grid[i][j] == '#') pos.pb(i,j);
  
  for_(i,0,sz(pos)) for_(j,i+1,sz(pos))
    ans += dist(pos[i].ff,pos[i].ss,pos[j].ff,pos[j].ss);
