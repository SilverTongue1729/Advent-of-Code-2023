...
void solve(){
  ...
  do {
    ...
    string game; iss>>game;
    int n; iss>>n;
    ans+=n;
    string colon; iss>>colon;
    int r=12,g=13,b=14;
    while(iss.peek() != EOF){
      int num; iss>>num;
      string color; iss>>color;
      string copy=color;
      if (color[sz(color)-1]==',' or color[sz(color)-1]==';') color.pop_back();
      
      if (color=="red") r-=num;
      else if (color=="green") g-=num;
      else if (color=="blue") b-=num;
      ...
      if (copy[sz(copy)-1]==';') r=12,g=13,b=14;      
    }
  } while (cin.peek() != EOF);
