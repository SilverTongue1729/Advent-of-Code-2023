...
void solve(){
  ...
  int hcur=0,hmin=0,hmax=0,vcur=0,vmin=0,vmax=0;
  for (auto& [c,i,str]:v) {
    switch (c) {
  ...
  int n = vmax-vmin+1+2, m = hmax-hmin+1+2;
  ...
  for (auto& [c,i,str]:v) {
    switch (c) {
      case 'R': for_(j,0,i) grid[x][++y] = 1; break;
      case 'L': for_(j,0,i) grid[x][--y] = 1; break;
      case 'U': for_(j,0,i) grid[--x][y] = 1; break;
      case 'D': for_(j,0,i) grid[++x][y] = 1; break;
    }
  }
  ...
  int ans = 0;
  for_(i,0,n) for_(j,0,m) ans += grid[i][j] != 2;
