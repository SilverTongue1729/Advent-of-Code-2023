...
template<typename Iter> bool is_unique(Iter begin, Iter end) { set<typename std::iterator_traits<Iter>::value_type> unique_set; for (Iter it = begin; it != end; ++it) if (!unique_set.insert(*it).second) return false; return true; }
...
struct Hand {
  ...
  bool operator<(const Hand& other) const {
    ...
    for (int i = 0; i < 5; ++i) {
      if (cardStrength[this->cards[i]] != cardStrength[other.cards[i]]) 
        return cardStrength[this->cards[i]] > cardStrength[other.cards[i]];
    }
    return false;
  }
  
 private:
  static HandType determineHandType(const string& cards) {
    map<char, int> cardFrequency;
    for (char card : cards) cardFrequency[card]++;
    int jokerCount = cardFrequency['J'];
    cardFrequency.erase('J');
    
    auto maxcnt = (*max_element(all(cardFrequency), [](const auto& p1, const auto& p2) { return p1.ss < p2.ss; })).ss;
    int pairCount = count_if(all(cardFrequency), [](const auto& p) { return p.ss == 2; });
        
    if (maxcnt + jokerCount == 5) return HandType::FiveOfAKind;
    if (maxcnt + jokerCount == 4) return HandType::FourOfAKind;
    if ((maxcnt == 3 and pairCount) or (pairCount == 2 and jokerCount)) return HandType::FullHouse;
    if (maxcnt == 3 or (pairCount and jokerCount) or jokerCount == 2) return HandType::ThreeOfAKind;
    if (pairCount == 2) return HandType::TwoPair;
    if (pairCount or jokerCount) return HandType::OnePair;
...
ostream& operator<<(ostream& out, const Hand& hand) {
  out << hand.cards << " " << static_cast<int>(hand.type);
  return out;
}

void solve() {
  vector<pair<Hand, ll>> hands;

  while (!cin.eof()) {
    string cards;
    ll bid;
    cin >> cards >> bid;
    hands.pb(Hand(cards), bid);
  }

  sort(all(hands));
    
  ll ans = 0, rank = hands.size();
