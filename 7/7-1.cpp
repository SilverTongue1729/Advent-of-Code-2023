...
struct Hand {
 ...
 private:
  static HandType determineHandType(const string& cards) {
    ...
    for (const auto& pair : cardFrequency) {
      if (pair.ss == 5) return HandType::FiveOfAKind;
      if (pair.ss == 4) return HandType::FourOfAKind;
      if (pair.ss == 3) isThree = true;
      if (pair.ss == 2) isTwo = true;
    }

    if (isThree and isTwo) return HandType::FullHouse;
    if (isThree) return HandType::ThreeOfAKind;
    if (isTwo) {
