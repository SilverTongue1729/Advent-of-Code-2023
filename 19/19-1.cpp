...
void solve(){
  ...
  for (auto& p : parts) {
    ...
    while (cur_map != "A" and cur_map != "R"){
      auto cur_wf = workflows[name_to_idx[cur_map]];
      
      cur_map = cur_wf.def_map;
      for (auto& [c,map]:cur_wf.conds) {
        int val=p[c.c];
        
        if (c.comp == 0 and val < c.val) {
          cur_map = map;
          break;
        }
        elif (c.comp == 1 and val > c.val) {
          cur_map = map;
          break;
        }
      }
    ...
    if (cur_map == "A") ans += p.x + p.m + p.a + p.s;
  }
