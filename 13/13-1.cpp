...
void solve(){
  ...
  while (!cin.eof()){
    ...
    auto is_same_col = [&](int c1, int c2) -> bool {
      for_(i,0,n) if (grid[i][c1] != grid[i][c2]) return false;
      return true;
    };
    auto is_same_row = [&](int r1, int r2) -> bool {
      for_(j,0,m) if (grid[r1][j] != grid[r2][j]) return false;
      return true;
    };
    ...
    for_(i,0,m-1){
      ...
      int c1=i,c2=i+1;
      while (c1>=0 and c2<m){
        if (!is_same_col(c1,c2)) { flag = false; break; }
        c1--; c2++;
      }
      if (flag) ans += i+1;
    }
    ...
    for_(i,0,n-1){
      ...
      int r1=i,r2=i+1;
      while (r1>=0 and r2<n){
        if (!is_same_row(r1,r2)) { flag = false; break; }
        r1--; r2++;
      }
      if (flag) ans += 100*(i+1);
    }
