...
void solve(){
  ...
  while (!cin.eof()){
    ...
    auto diff_col = [&](int c1, int c2) -> int {
      int diff=0;
      for_(i,0,n) if (grid[i][c1] != grid[i][c2]) diff++;
      return diff;
    };
    auto diff_row = [&](int r1, int r2) -> int {
      int diff=0;
      for_(i,0,m) if (grid[r1][i] != grid[r2][i]) diff++;
      return diff;
    };
